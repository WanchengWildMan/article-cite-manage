<template>
  ...
  <div>
     <el-form-item
            ...
            :rules="rules[item.name]!=null ? rules : {}"
          >
            ...
            <el-select v-model="scope.row[item.name]" v-if="item.isSelect">
              <el-option
    ...
    <el-table :data="tableData"
              border
              style="widt h: 100%; border-radius:20px; "
  ...
  methods: {
    ...
    page(currentPage) {
      ...
      axios.get(`http://localhost:${this.port}/admin/find`).then(function (resp) {
        ...
        _this.tableData = resp.data.content.slice((currentPage - 1) * _this.pageEachSize, currentPage * _this.pageEachSize);
        console.log(_this.tableData)
        _this.pageNum = Math.ceil(resp.data.content.length / _this.pageEachSize);
